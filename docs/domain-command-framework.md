...
| `account-operation +list` | read | — | — | direct | List operated accounts in the workspace, one page at a time. This is paginated: absence from one page is NOT evidence that a pool account is unmanaged; use +ops-status-accounts for exact known pool-account ids. |
