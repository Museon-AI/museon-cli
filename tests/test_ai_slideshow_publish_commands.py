"""Focused coverage for canonical ai-slideshow publish batch schedule commands."""
...
def test_plan_builder_rejects_total_occurrence_budget() -> None:
    ...
    for number in range(29):
        argv[3:3] = ["--account-id", f"10000000-0000-4000-8000-{number:012d}"]
...
def test_single_account_asset_commands_are_not_reintroduced() -> None:
    with pytest.raises(ValueError, match="Unknown command schema"):
        get_command_spec("social-account.assets-get")
