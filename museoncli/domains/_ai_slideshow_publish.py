"""AI slideshow publishing commands for durable batch schedule planning."""
...
def _build_asset_pool_account_ids(args: argparse.Namespace) -> list[str]:
    ...
    if len(account_ids) > ACCOUNT_PUBLISH_MAX_ACCOUNTS:
        raise ValueError(
            "ai-slideshow publish asset-pool batching accepts at most "
...
def _build_schedule_plan_arguments(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if len(account_ids) > ACCOUNT_PUBLISH_MAX_ACCOUNTS:
        raise ValueError(
            "ai-slideshow publish schedule planning accepts at most "
            f"{ACCOUNT_PUBLISH_MAX_ACCOUNTS} --account-id values."
        )
    ...
    return [
        CommandSpec(
            ...
            summary=(
                ...
                "rather than issuing one request per account. This batch command is also valid for a complete "
